      ...
      getAPI(url).then((response) => {
        if (response.data.sessionLocation.length !== 0) {
          ...
          for (let i = 0; i < response.data.sessionLocation.length; i++) {
            ...
            let district_name =
              searchdatanew[0][i]["online_hospital_district"][
      ...
      {isChart && (
        <Paper className={classes.paper}>
          <Chart
            ...
            data={allHosptsData}
            labelName={allHosptsName}
            maleData={allHosptsmale}
            femaleData={allHosptsfemale}
          />
        </Paper>
      )}
    </>
